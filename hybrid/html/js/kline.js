...
var app = new Vue({
	...
	mounted() {
		...
		// 	if(event.action==="update_kline"){
		// 		console.log('接收到的消息:', event.data);
		// 		this.updateChartLine(event.data);
		// 	}
		// });
		let that = this
			window.addEventListener('message', function(event) {
					console.log('接收到的消息44:', event.data);
					const msg = event.data;
					if(msg.type==="update_kline"){
						console.log(msg.data)
						that.updateChartLine(msg.data)
					}
				});
	...
	methods:{
		...
		// 获取k线数据,生成k线
		updateChartLine(udata){
			 dates = udata.map(function(item) {
				return item[0];
			});
			 data = udata.map(function(item) {
				return [+item[1], +item[2], +item[3], +item[4], +item[5]];
			});
			 volumes = udata.map(function(item, index) {
